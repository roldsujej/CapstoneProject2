...
if (isset($_POST['upload'])) {
    ...
    if (isset($_FILES['uploadedFile'])) {
        ...
        if ($file['error'] === UPLOAD_ERR_OK) {
            ...
            // Check if the applicant has already uploaded a document for this requirement
            $checkQuery = "SELECT id FROM uploaded_documents WHERE applicant_id = ? AND document_id = ?";
            $checkStmt = $conn->prepare($checkQuery);
            $checkStmt->bind_param("ii", $applicantId, $documentId);
            $checkStmt->execute();
            $checkStmt->store_result();

            if ($checkStmt->num_rows > 0) {
                $_SESSION['status'] = "You have already uploaded a document for this requirement.";
